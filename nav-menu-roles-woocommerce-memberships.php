...
function nmr_wcm_init(){
    if ( defined( 'YITH_WCMBS_VERSION' ) ){
		add_filter( 'nav_menu_roles', 'nmw_yith_wcm_new_roles' );
		add_filter( 'nav_menu_roles_item_visibility', 'nmw_yith_wcm_item_visibility', 10, 2 );
    }
}
 ...
 * @since 1.0.0
 */
function nmw_yith_wcm_new_roles( $roles ){
	return array_merge( $roles, nmr_yith_wcm_get_roles_wrapper() );
}
 ...
 * @since 1.0.0
 */
function nmw_yith_wcm_item_visibility( $visible, $item ){
...
/*
 * Get the plugin-specific "roles", from YITH Woo Commerce plugin, returned in an array, with ID => Name key pairs
 *
 * @return array
 * @since 1.1.0
 */
function nmr_yith_wcm_get_roles_wrapper(){
    $roles = array();
    ...
    if( ! empty( $plans ) ) {
        foreach( $plans as $plan ){
            $roles['wc_membership_' . $plan->ID] = $plan->post_title;
        }
    }
